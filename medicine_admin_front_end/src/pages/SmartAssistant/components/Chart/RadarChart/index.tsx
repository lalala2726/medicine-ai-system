...
/**
 * 自定义雷达图组件
 * 用于在 Markdown 中渲染 radar 代码块
 * 用于展示多个维度的数据对比
 */
const RadarChart: React.FC<RadarChartProps> = ({ children }) => {
  ...
  try {
    ...
    return (
      <div style={{ width: '100%', minHeight: height, overflow: 'hidden' }}>
        <Radar data={data} containerStyle={{ height, width: '100%' }} theme={theme} style={style} />
      </div>
    );
  } catch {
