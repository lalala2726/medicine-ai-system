...
后来有一次我生病了，是上呼吸道感染，喉咙特别难受，当时就急着去买药。但问题是我自己也不知道该买什么药，就在买药在线问诊。那次挂号原本是 20 元，有优惠还便宜了10元。也是那次经历之后，我突然想到：能不能用 AI 做一个可以一步步追问症状、辅助判断情况，再推荐合适药品的系统？
...
最近临近毕业季，一边要写论文，所以很多更具体的实现步骤和细节文档后续会慢慢补上。现在更建议先通过上面的在线地址实际体验一下项目。
...
- 同步管理端智能体观测模块，补充监控面板、智能体跟踪详情、权限路由和相关交互优化。
- 同步后端 Agent Trace 监控查询接口、图表视图、可读消息视图和运行跟踪管理能力。
- 同步 AI Agent 链路追踪序列化、顶层消息快照、错误信息字段和上下文缓存能力优化。
- 同步客户端助手交互、深度思考展示、Markdown 展示和结算链路相关优化。
