import { baseRequestClient } from '@/request/baseRequestClient'
 ...
 * 后端 AjaxResult 原始结构。
 */
interface RawCaptchaResponse<TData = unknown> {
  /** 业务状态码 */
  code?: number
  /** 后端 message 字段 */
  message?: string
...
export async function requestCaptcha<TResponseData = unknown, TRequestData = unknown>(
  ...
  if (method === 'GET') {
    rawResponse = await baseRequestClient.get<RawCaptchaResponse<TResponseData>>(payload.url, {
      headers: payload.headers
    })
  } else {
    rawResponse = await baseRequestClient.request<RawCaptchaResponse<TResponseData>>({
      url: payload.url,
      method,
      data: payload.data,
      headers: payload.headers
