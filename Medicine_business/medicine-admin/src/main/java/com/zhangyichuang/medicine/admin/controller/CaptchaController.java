...
@Tag(name = "管理端验证码接口", description = "管理端滑块验证码接口")
public class CaptchaController {
    ...
    @Anonymous
    @GetMapping("/gen")
    @Operation(summary = "生成滑块验证码", description = "生成管理端滑块验证码 challenge")
    ...
    @Anonymous
    @PostMapping("/check")
    @Operation(summary = "校验滑块验证码", description = "校验管理端滑块验证码轨迹")
    ...
    private <T> AjaxResult<T> convertCaptchaResponse(ApiResponse<T> captchaResponse) {
        ...
        return AjaxResult.error(ResponseCode.PARAM_ERROR.getCode(), StringUtils.hasText(captchaResponse.getMsg()) ? captchaResponse.getMsg() : "验证码校验失败");
    }
}
