...
/**
 * 自定义缩进树组件
 * 用于在 Markdown 中渲染 indentedtree 代码块
 * 用于展示层级结构、文件树、知识树等
 */
const IndentedTreeChart: React.FC<IndentedTreeChartProps> = ({ children }) => {
  ...
  try {
    ...
    // 验证数据结构
    if (!data.name) {
      return <div style={{ padding: 20, color: '#ff4d4f' }}>错误:缩进树数据必须包含 name 字段</div>;
    }
    ...
    return (
      ...
      >
        <IndentedTree data={data} theme={theme} containerStyle={{ height, width: '100%' }} />
      </div>
    );
  } catch {
...
export default IndentedTreeChart;
